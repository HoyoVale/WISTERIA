...
void TestMorphLabDemoAsset()
{
    ...
    Scene scene;
    Entity& reference = scene.InstantiateModel(model, Transform{});
    Entity& active = scene.InstantiateModel(model);
    Require(
        !reference.HasPhysicsInstance() && !active.HasPhysicsInstance() &&
        scene.Physics().BodyCount() == 0U &&
        scene.Physics().ConstraintCount() == 0U,
        "Morph Lab model instantiation attached an unexpected physics runtime"
    );
