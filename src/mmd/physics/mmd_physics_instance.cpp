...
MmdPhysicsInstance::MmdPhysicsInstance(
    ...
    : world(&world),
      ...
      runtimePolicy(std::move(policy)),
      gravityMode(this->runtimePolicy.initialGravityMode),
      physicsWithBoneSyncMode(this->runtimePolicy.initialSyncMode)
{
...
MmdPhysicsInstance::StabilizationRequest() const noexcept
{
    return PhysicsStabilizationRequest{
        ...
        this->runtimePolicy.stabilization.fixedTimeStep
    };
}
...
void MmdPhysicsInstance::CompleteStabilization()
{
    ...
    const bool failed = !finalSnapshot.finite ||
        finalSnapshot.maximumStabilizationLinearViolation >
            this->runtimePolicy.stabilization.failureLinearViolation ||
        finalSnapshot.maximumStabilizationAngularViolationDegrees >
            this->runtimePolicy.stabilization.failureAngularViolationDegrees;
...
const MmdPhysicsRuntimePolicy&
MmdPhysicsInstance::RuntimePolicy() const noexcept
{
    ...
    if (this->resetTargetRefreshPending)
    {
        ...
        const bool needsWarmup = !refreshed.finite ||
            refreshed.maximumStabilizationLinearViolation >
                this->runtimePolicy.stabilization.warmupLinearViolation ||
            refreshed.maximumStabilizationAngularViolationDegrees >
                this->runtimePolicy.stabilization.warmupAngularViolationDegrees;
        this->pendingStabilizationSteps = needsWarmup
            ? this->runtimePolicy.stabilization.steps
            : 0U;
        this->BuildAlignmentDiagnostics();
        if (this->rigidBodies.size() >= 32U)
        {
...
void MmdPhysicsInstance::ConfigureGravityBalanceProfiles()
{
    ...
    for (std::size_t start = 0U; start < bodyCount; ++start)
    {
        ...
        const MmdPhysicsChainTuning& profile =
            this->runtimePolicy.ChainTuning(resolvedKind);
        GravityChain chain;
...
void MmdPhysicsInstance::ApplyGravityBalanceSettings(bool force)
{
    ...
    const bool profilesEnabled = this->runtimePolicy.enableChainProfiles &&
...
void MmdPhysicsInstance::UpdateGravityBalanceStatistics()
{
    ...
    this->gravityStatistics.chainProfilesEnabled =
        this->runtimePolicy.enableChainProfiles &&
        GravityModeUsesChainProfiles(this->gravityMode);
    this->gravityStatistics.chainCount = this->gravityChains.size();
...
void MmdPhysicsInstance::ConfigureCollisionTopology()
{
         ...
         ++bodyAIndex)
    {
        ...
        const std::size_t maximumDepth = skirtChain
            ? this->runtimePolicy.collision.skirtSelfCollisionGraphDistance
            : 2U;
             ...
             ++bodyBIndex)
        {
            ...
            const bool graphCandidate =
                this->runtimePolicy.collision.enableNearNeighborFiltering &&
                graphDistance >= 2U && graphDistance <= maximumDepth;
            bool semanticConflict = false;
            if (this->runtimePolicy.collision.enableSkirtSemanticFiltering &&
                skirtChain &&
            ...
            const float proximityFactor = skirtChain
                ? this->runtimePolicy.collision.skirtSelfCollisionProximityFactor
                : this->runtimePolicy.collision.nearNeighborProximityFactor;
            if (!semanticConflict &&
                glm::distance(positionA, positionB) >
...
{
    this->collisionStatistics.activeCcdBodyCount = 0U;
    if (!this->runtimePolicy.ccd.adaptive)
    {
        for (RuntimeBody& runtime : this->rigidBodies)
        {
            if (!runtime.ccdCandidate || !runtime.ccdActive)
                continue;
            this->world->ConfigureCcd(runtime.handle, false, 0.0f, 0.0f);
            runtime.ccdActive = false;
            runtime.ccdIdleSeconds = 0.0f;
    ...
    for (RuntimeBody& runtime : this->rigidBodies)
    {
        ...
        const float enableTravel = std::max(
            ...
            runtime.ccdFeatureSize * this->runtimePolicy.ccd.enableTravelFactor
        );
        const float disableTravel = runtime.ccdFeatureSize *
            this->runtimePolicy.ccd.disableTravelFactor;
        ...
        else if (runtime.ccdActive)
        {
            ...
            if (runtime.ccdIdleSeconds >= this->runtimePolicy.ccd.disableDelaySeconds)
            {
                this->world->ConfigureCcd(
                    runtime.handle,
                    false,
...
void MmdPhysicsInstance::UpdateCollisionDiagnostics()
{
    ...
    for (const PhysicsContactPair& pair : this->world->ContactPairs())
    {
        ...
        if (this->contactDiagnostics.size() < this->runtimePolicy.collision.maximumContactDiagnostics)
...
{
    if (!this->runtimePolicy.recovery.enabled ||
        this->recoveryChains.empty())
        return;
    ...
    for (std::size_t index = 0U; index < this->rigidBodies.size(); ++index)
    {
        ...
        if (linearSpeed > this->runtimePolicy.recovery.hardLinearSpeed * scale ||
            angularSpeed > this->runtimePolicy.recovery.hardAngularSpeed)
        {
            RecoveryTrigger trigger;
            ...
            trigger.score = 900.0f + std::max(
                linearSpeed / (this->runtimePolicy.recovery.hardLinearSpeed * scale),
                angularSpeed / this->runtimePolicy.recovery.hardAngularSpeed
            );
            mark(chainIndex, trigger);
        }
        else if (linearSpeed > this->runtimePolicy.recovery.linearSpeed * scale ||
            angularSpeed > this->runtimePolicy.recovery.angularSpeed)
        {
            RecoveryTrigger trigger;
            ...
            trigger.score = 300.0f + std::max(
                linearSpeed / (this->runtimePolicy.recovery.linearSpeed * scale),
                angularSpeed / this->runtimePolicy.recovery.angularSpeed
            );
            mark(chainIndex, trigger);
        }
        ...
        const float runawayDistance = std::max(
            this->runtimePolicy.recovery.runawayDistance * scale,
            bodySize * 12.0f
        );
        ...
        const bool extensionGrowing = anchorMetricValid &&
            normalizedExtension >
                previousExtension + this->runtimePolicy.recovery.extensionGrowthTolerance;
        const bool supportedByMotion =
            linearSpeed > this->runtimePolicy.recovery.runawaySupportSpeed * scale;
        const bool hardExtension = anchorMetricValid &&
            normalizedExtension > this->runtimePolicy.recovery.hardNormalizedExtension;
        const bool sustainedExtension = anchorMetricValid &&
            normalizedExtension > this->runtimePolicy.recovery.normalizedExtension &&
        ...
        if (hardExtension || sustainedExtension)
        {
            ...
            trigger.immediate = normalizedExtension >
                this->runtimePolicy.recovery.hardNormalizedExtension * 2.0f;
    ...
    for (std::size_t jointIndex = 0U; jointIndex < joints.size(); ++jointIndex)
    {
        ...
        const float severity = std::max({
            positionError / (this->runtimePolicy.recovery.jointSeparation * scale),
            maximumLinearViolation / (this->runtimePolicy.recovery.linearViolation * scale),
            maximumAngularViolationDegrees /
                this->runtimePolicy.recovery.angularViolationDegrees
        });
        ...
        const bool hard =
            positionError > this->runtimePolicy.recovery.hardJointSeparation * scale ||
            maximumLinearViolation >
                this->runtimePolicy.recovery.hardLinearViolation * scale ||
            maximumAngularViolationDegrees >
                this->runtimePolicy.recovery.hardAngularViolationDegrees;
        const bool sustainedCandidate = severity > 1.0f &&
            (severity >= 1.5f ||
                previousSeverity >= 1.0f ||
                severity > previousSeverity + this->runtimePolicy.recovery.severityGrowthTolerance);
         ...
         ++chainIndex)
    {
        ...
        const float persistence =
            trigger.reason == MmdPhysicsRecoveryReason::HighVelocity
                ? this->runtimePolicy.recovery.highVelocityPersistenceSeconds
                : this->runtimePolicy.recovery.persistenceSeconds;
        ...
        chain.abnormalSeconds = 0.0f;
        chain.pendingTrigger = {};
        chain.cooldownSeconds = this->runtimePolicy.recovery.cooldownSeconds;
        chain.fuseSuppressionLatched = false;

        if (chain.fuseWindowSeconds <= 0.0f)
        {
            chain.fuseWindowSeconds = this->runtimePolicy.recovery.fuseWindowSeconds;
            chain.recoveriesInWindow = 0U;
        }
        ++chain.recoveriesInWindow;
        if (chain.recoveriesInWindow >= this->runtimePolicy.recovery.fuseLimit)
        {
            chain.fuseRemainingSeconds = this->runtimePolicy.recovery.fuseDurationSeconds;
            ++this->recoveryStatistics.totalFuseTrips;
            std::cout << "[MMD RECOVERY FUSE] chain=" << chainIndex
                      << " tripped=true durationMs="
                      << this->runtimePolicy.recovery.fuseDurationSeconds * 1000.0f
                      << " recoveriesInWindow="
                      << chain.recoveriesInWindow
                      << " totalTrips="
                      << this->recoveryStatistics.totalFuseTrips
...
) const
{
    ...
    while (!pending.empty() && region.size() < this->runtimePolicy.recovery.maximumTotalBodies)
    {
        const auto [bodyIndex, depth] = pending.front();
        pending.pop();
        const bool dynamic = this->rigidBodies[bodyIndex].definition->mode !=
            MmdRigidBodyMode::FollowBone;
        if (dynamic && dynamicBodyCount >= this->runtimePolicy.recovery.maximumDynamicBodies)
            continue;

        region.push_back(bodyIndex);
        if (dynamic)
            ++dynamicBodyCount;
        if (depth >= this->runtimePolicy.recovery.localGraphRadius)
            continue;
        if (!dynamic && depth > 0U)
            continue;
...
) const
{
    ...
    for (std::size_t index = 0U; index < joints.size(); ++index)
    {
        ...
        if (maximumLinearViolation > this->runtimePolicy.stabilization.failureLinearViolation ||
            maximumAngularViolationDegrees >
                this->runtimePolicy.stabilization.failureAngularViolationDegrees)
        {
            ++snapshot.jointsOverFailureThreshold;
