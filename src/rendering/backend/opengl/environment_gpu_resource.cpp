...
void AllocateCubemapLevel(unsigned int resolution, unsigned int mipLevel)
{
...
void EnvironmentMapGpuResource::Attach()
{
    if (this->attached)
        return;
...
bool EnvironmentMapGpuResource::IsAttached() const noexcept
{
    return this->attached;
}

void EnvironmentMapGpuResource::BindIrradiance(unsigned int unit) const
{
    if (!this->attached)
...
void EnvironmentMapGpuResource::BindPrefilter(unsigned int unit) const
{
    if (!this->attached)
        throw std::logic_error("Environment map must be attached before binding");
    glActiveTexture(GL_TEXTURE0 + unit);
...
void EnvironmentMapGpuResource::BindBrdfLut(unsigned int unit) const
{
    if (!this->attached)
        throw std::logic_error("Environment map must be attached before binding");
    glActiveTexture(GL_TEXTURE0 + unit);
...
void EnvironmentMapGpuResource::ConfigureSkyboxVertexArray(
    VAO& vertexArray
) const
{
...
void EnvironmentMapGpuResource::DrawSkybox(
    const glm::mat4& view,
...
)
{
    ...
    this->skyboxProgram->Uniform1f("environmentIntensity", liveData.intensity);
...
void EnvironmentMapGpuResource::CreateGeometry()
{
...
void EnvironmentMapGpuResource::CreateIrradianceMap()
{
...
void EnvironmentMapGpuResource::CreatePrefilterMap()
{
...
void EnvironmentMapGpuResource::CreateBrdfLut()
{
...
void EnvironmentMapGpuResource::CreateSkyboxProgram()
{
...
void EnvironmentMapGpuResource::RenderCube() const
{
    glBindVertexArray(this->cubeVao);
    glDrawArrays(GL_TRIANGLES, 0, 36);
}

void EnvironmentMapGpuResource::RenderQuad() const
{
    glBindVertexArray(this->quadVao);
